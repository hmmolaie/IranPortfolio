...
تلگرام: ربات عمومی `https://t.me/sabadyaar_bot`؛ تنظیمات ادمین در `telegram`؛ پیام ۸:۳۰ به کاربرانی که موبایل پروفایل دارند و ربات را وصل کرده‌اند. اخبار و فرصت‌ها علاوه بر متن، به‌صورت فایل صوتی mp3 فارسی با صدای زن و حداکثر حدود دو دقیقه (`digest-voice.ts` + پرامپت `telegram_digest_voice` + `speakTts`) با `sendAudio` فرستاده می‌شود؛ صدا از `TTS_VOICE` خوانده می‌شود و فقط مقادیر زنانه پذیرفته است.
