...
دستیار تلگرام: ربات دوطرفه جدا (`telegram-assistant`) برای پرسش، ترجمهٔ صفحه، PDF فارسی RTL، و یوتیوب. لینک یوتیوب ویدئو را با `yt-dlp` می‌گیرد؛ اگر زیرنویس انگلیسی نباشد صدا با مدل ثابت `gapgpt/whisper-1` روی مسیر `/v1/audio/transcriptions` متن زمان‌دار می‌شود، LLM همان قطعه‌ها را بدون تغییر زمان ترجمه می‌کند، زیرنویس فارسی با ffmpeg روی تصویر می‌سوزد و `video-fa.mp4` برمی‌گردد. «خلاصه کن» همان مسیر صوتی قبلی است.
